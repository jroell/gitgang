...
async function git(cwd: string, ...args: string[]): Promise<string> {
  ...
  await proc.exited;
  if (proc.exitCode !== 0) {
    throw new Error(`git ${args.join(" ")} failed (exit ${proc.exitCode}): ${stderr || stdout}`);
  }
  return stdout.trim();
}
...
async function repoRoot(): Promise<string> { 
  ...
  await proc.exited;
  if (proc.exitCode !== 0) throw new Error("Not in a git repository");
  return stdout.trim();
}
...
async function runCodexCoder(w: Worktree, base: string, task: string, yolo: boolean): Promise<ProcWrap> { const prompt=`${systemConstraints("codex")}\n\n${featurePrompt("codex", base, task)}`; const args=["exec",prompt,"--model",MODELS.codex,"--config","model_reasoning_effort=\"high\""]; args.push(yolo?"--yolo":"--full-auto"); const proc=spawn(["codex",...args],{cwd:w.dir,stdout:"pipe",stderr:"pipe"});  streamToLog(TAG("codex"), w.log, C.green, proc.stdout!); streamToLog(TAG("codex"), w.log, C.green, proc.stderr!); return { proc, name:"codex", log:w.log }; }
async function runCodexReviewer(cwd:string, base:string, branches:{gemini:string; claude:string; codex:string;}, task:string, yolo:boolean){ const prompt=reviewerPromptJSON(base, branches, task); const args=["exec",prompt,"--model",MODELS.codex,"--config","model_reasoning_effort=\"high\""]; args.push(yolo?"--yolo":"--full-auto"); return await spawn(["codex",...args],{cwd,stdout:"pipe",stderr:"pipe"}); }
...
async function doReview(opts: Opts, w: Record<AgentId, Worktree>) {
  ...
  if (decision.status === "approve") {
    ...
    if (opts.autoPR) {
      try { 
        const proc = Bun.spawn(["gh", "pr", "create", "--fill", "--base", opts.baseBranch, "--head", mergeBranch], { cwd: opts.repoRoot, stdout: "pipe", stderr: "pipe" });
        await proc.exited;
        if (proc.exitCode === 0) {
          console.log(C.green("PR created."));
        } else {
          console.log(C.yellow("GitHub CLI not configured or failed – skipping PR."));
        }
      } catch { 
        console.log(C.yellow("GitHub CLI not configured or failed – skipping PR.")); 
      }
