...
import { mkdirSync, mkdtempSync, rmSync, existsSync, readFileSync, writeFileSync, chmodSync } from "node:fs";
...
describe("CLAUDE.md bootstrap", () => {
  ...
  test("bootstrapClaudeMd preserves existing CLAUDE.md and appends context", () => {
    ...
    // Should append gitgang context
    expect(content).toContain("# Environment Context (auto-generated by gitgang)");
    // Should include test scripts
  ...
  test("bootstrapClaudeMd includes commit-early ground rule", () => {
    writeFileSync(join(tmpDir, "test_check.sh"), "echo pass");
    bootstrapClaudeMd(tmpDir);
    const content = readFileSync(join(tmpDir, "CLAUDE.md"), "utf-8");
    expect(content).toContain("Commit EARLY");
  });
});
...
describe("findSourceFiles", () => {
  let tmpDir: string;
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "gitgang-test-srcfiles-"));
  });
  afterEach(() => {
    rmSync(tmpDir, { recursive: true, force: true });
  });

  test("finds source files with common extensions", () => {
    writeFileSync(join(tmpDir, "main.py"), "print('hello')");
    writeFileSync(join(tmpDir, "app.js"), "console.log('hi')");
    writeFileSync(join(tmpDir, "solution.rs"), "fn main() {}");
    const files = findSourceFiles(tmpDir);
    expect(files.length).toBe(3);
    expect(files.map(f => f.relative)).toContain("main.py");
    expect(files.map(f => f.relative)).toContain("app.js");
    expect(files.map(f => f.relative)).toContain("solution.rs");
  });

  test("skips test files", () => {
    writeFileSync(join(tmpDir, "main.py"), "print('hello')");
    writeFileSync(join(tmpDir, "test_main.py"), "assert True");
    writeFileSync(join(tmpDir, "main.test.js"), "test()");
    const files = findSourceFiles(tmpDir);
    expect(files.length).toBe(1);
    expect(files[0].relative).toBe("main.py");
  });

  test("skips lock files and minified files", () => {
    writeFileSync(join(tmpDir, "main.js"), "console.log('hi')");
    writeFileSync(join(tmpDir, "package-lock.json"), "{}");
    writeFileSync(join(tmpDir, "app.min.js"), "minified");
    const files = findSourceFiles(tmpDir);
    expect(files.length).toBe(1);
    expect(files[0].relative).toBe("main.js");
  });

  test("skips files larger than 15KB", () => {
    writeFileSync(join(tmpDir, "small.py"), "x = 1");
    writeFileSync(join(tmpDir, "large.py"), "x = 1\n".repeat(5000));
    const files = findSourceFiles(tmpDir);
    expect(files.length).toBe(1);
  ...
  test("skips node_modules and .git directories", () => {
    ...
    writeFileSync(join(tmpDir, "node_modules", "lib.js"), "module");
    writeFileSync(join(tmpDir, ".git", "config.py"), "git");
    writeFileSync(join(tmpDir, "main.py"), "real code");
    const files = findSourceFiles(tmpDir);
    expect(files.length).toBe(1);
    expect(files[0].relative).toBe("main.py");
  });
...
describe("runPreflightTests", () => {
  let tmpDir: string;
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "gitgang-test-preflight-"));
  });
  afterEach(() => {
    rmSync(tmpDir, { recursive: true, force: true });
  });

  test("returns null when no test files and no common test commands", () => {
    const result = runPreflightTests(tmpDir, []);
    expect(result).toBeNull();
  });

  test("runs executable test scripts and captures output", () => {
    const scriptPath = join(tmpDir, "test_check.sh");
    writeFileSync(scriptPath, "#!/bin/bash\necho 'PASS: all tests passed'");
    chmodSync(scriptPath, 0o755);
    const result = runPreflightTests(tmpDir, [
      { path: scriptPath, relative: "test_check.sh" },
    ]);
    expect(result).toContain("PASS: all tests passed");
  });

  test("captures test failure output", () => {
    const scriptPath = join(tmpDir, "test_fail.sh");
    writeFileSync(scriptPath, "#!/bin/bash\necho 'FAIL: expected 42 got 0' && exit 1");
    chmodSync(scriptPath, 0o755);
    const result = runPreflightTests(tmpDir, [
      { path: scriptPath, relative: "test_fail.sh" },
    ]);
    expect(result).toContain("FAIL: expected 42 got 0");
  });

  test("skips non-executable non-shebang files", () => {
    const scriptPath = join(tmpDir, "test_data.py");
    writeFileSync(scriptPath, "# just a data file\nexpected = 42");
    // No exec bit, no shebang
    const result = runPreflightTests(tmpDir, [
      { path: scriptPath, relative: "test_data.py" },
    ]);
    expect(result).toBeNull();
  });
});
