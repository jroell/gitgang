...
gg --solo claude "Add user authentication"   # single-agent run, no reviewer loop
```

`--solo <agent>` runs only that agent, skips the reviewer loop, and auto-merges the resulting branch directly when the merge is clean.
